...
def fit_per_regime(returns, regime_seq, n_regimes, assets, min_obs=60):
    """
    For each regime: fit per-asset GARCH + DCC + t-copula.

    Returns:
        regime_models: list of dicts
    """
    print("\nFitting per-regime GARCH + DCC + t-copula...")
    regime_models = []
...
def simulate_hybrid_paths(
    ...
    if return_prices:
        ...
        return all_returns, all_regimes, all_prices

    return all_returns, all_regimes
...
def main():
    ...
    all_returns, all_regimes, all_prices = simulate_hybrid_paths(
