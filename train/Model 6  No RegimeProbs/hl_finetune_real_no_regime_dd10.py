"""
train/hl_finetune_real_no_regime.py

HL fine-tune on real_train, starting from the no-regime HL synth pretrain
endpoint. Frozen LL = ll_ft_no_regime_final (no-regime fine-tuned LL).

Mirrors the original hl_finetune_real_unconstrained.py protocol that
produced the project's best HRL result.

Run from project root:
    python train/hl_finetune_real_no_regime.py
"""
...
HL_PRETRAIN_DIR = PATHS.checkpoints / "hl_synth_pretrain_no_regime"
HL_MODEL_PATH = HL_PRETRAIN_DIR / "hl_no_regime_final.zip"
HL_VECNORM_PATH = HL_PRETRAIN_DIR / "hl_no_regime_final_vecnorm.pkl"
    ...
    def _make(rank):
        def _init():
            core = PortfolioCore(
                ...
                rng=np.random.default_rng(16000 + rank),
            )
...
def main():
    print("=" * 80)
    print("HL fine-tune on real_train — NO regime probs")
    print("=" * 80)
    ...
    print(f"\nStarting from HL: {HL_MODEL_PATH.name}")
    ...
    ckpt_dir = PATHS.checkpoints / "hl_finetune_real_no_regime_dd10"
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    tb_dir = PATHS.tb_logs / "hl_finetune_real_no_regime_dd10"
    ...
    checkpoint_cb = CheckpointCallback(
        ...
        name_prefix="ppo_hl_ft_no_regime",
    ...
    try:
        ...
        final_model_path = ckpt_dir / "hl_ft_no_regime_final.zip"
        final_vecnorm_path = ckpt_dir / "hl_ft_no_regime_final_vecnorm.pkl"
    ...
    print()
    print("=" * 80)
    print("HL fine-tune (no regime) complete.")
    print("=" * 80)
