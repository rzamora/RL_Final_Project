"""
train/hl_synth_pretrain_v2.py

HL synth pretrain v2 — uses the random-HL-trained LL as frozen worker.

Difference from train/hl_synth_pretrain.py:
  - Frozen LL is now ll_random_hl_finetune (LL trained with HL action
    sampled Uniform(-1,+1)^2 per episode, then fine-tuned on real train).
  - Fix A patch removed: the new LL's VecNormalize stats already cover
    the full HL action distribution, so we use them as-is.

Hypothesis: with an LL that genuinely conditions on HL action, the HL has
a real gradient signal for "be defensive in SevereBear, aggressive in Bull"
and posture/net_gap_SB_minus_Bull will move meaningfully negative over
training. The original v1 had this gap stuck near 0 throughout.

Run from project root:
    python train/hl_synth_pretrain_v2.py
"""
...
# ---------------------------------------------------------------------------
# Config — frozen LL is the random-HL fine-tuned checkpoint
# ---------------------------------------------------------------------------
...
PPO_KWARGS = dict(
    ...
    gae_lambda=0.95,
    clip_range=0.1,
    ent_coef=0.03,  # <==== changed from 0.01
    n_epochs=4,
    device="cpu",
    seed=0,
    verbose=1,
...
def load_frozen_ll():
    """Load the random-HL LL and its VecNormalize stats. Unlike v1, NO Fix A
    patch — the new LL's VecNormalize already covers the full HL action
    distribution."""
    ...
    # NO Fix A patch — the LL was trained with random HL actions so its
    # action-slot VecNormalize stats are already meaningful
...
class PerRegimePostureCallback(BaseCallback):
    """Same as v1 — every eval_freq steps, runs deterministic episode on
    real test from day 0 and logs per-regime mean gross and net.
    """
...
def main():
    print("=" * 80)
    print("HL synth pretrain v2 — frozen LL = random-HL fine-tuned")
    print("=" * 80)
    ...
    ckpt_dir = PATHS.checkpoints / "hl_synth_pretrain_v2_constrained_gross"
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    tb_dir = PATHS.tb_logs / "hl_synth_pretrain_v2_constrained_gross"
    ...
    try:
        ...
        final_model_path = ckpt_dir / "hl_v2_cg_final.zip"
        final_vecnorm_path = ckpt_dir / "hl_v2_cg_final_vecnorm.pkl"
    ...
    print()
    print("=" * 80)
    print("HL synth pretrain v2 complete.")
    print("=" * 80)
