"""
train/hl_finetune_real_constrained_gross.py

HL fine-tune on real_train data, starting from the constrained-gross
HL ent=0.03 1M synth pretrain endpoint.

Identical to hl_finetune_real_unconstrained.py except:
  - Imports HighLevelPortfolioEnv from portfolio_hrl_env_constrained_gross
    (with parse_hl_action mapping gross_signal to [0.8, 1.0])
  - Loads from hl_synth_pretrain_v2_constrained_gross checkpoint
  - Saves to hl_finetune_real_constrained_gross output directory

Run from project root:
    python train/hl_finetune_real_constrained_gross.py
"""
...
# Starting checkpoint: HL ent=0.03 constrained-gross 1M final
HL_PRETRAIN_DIR = PATHS.checkpoints / "hl_synth_pretrain_v2_constrained_gross"
HL_MODEL_PATH = HL_PRETRAIN_DIR / "hl_v2_cg_final.zip"
HL_VECNORM_PATH = HL_PRETRAIN_DIR / "hl_v2_cg_final_vecnorm.pkl"
    ...
    def _make(rank):
        def _init():
            core = PortfolioCore(
                ...
                rng=np.random.default_rng(8500 + rank),  # different seed range from unconstrained
            )
...
def main():
    print("=" * 80)
    print("HL fine-tune on real_train — CONSTRAINED GROSS [0.8, 1.0]")
    print("=" * 80)
    ...
    ckpt_dir = PATHS.checkpoints / "hl_finetune_real_constrained_gross"
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    tb_dir = PATHS.tb_logs / "hl_finetune_real_constrained_gross"
    ...
    try:
        ...
        final_model_path = ckpt_dir / "hl_ft_cg_final.zip"
        final_vecnorm_path = ckpt_dir / "hl_ft_cg_final_vecnorm.pkl"
    ...
    print()
    print("=" * 80)
    print("HL fine-tune (constrained gross) complete.")
    print("=" * 80)
