"""
train/hl_synth_pretrain_v3.py

HL synth pretrain v3 — frozen LL = ll_regime_bucket_hl_finetune (100k).

Versioning recap:
  v1: frozen LL = light_100k (regime-blind LL trained with fixed [0.33, 0.5]).
      Result: net_gap_SB_minus_Bull stuck at 0 throughout 1M steps. HL never
      learned regime-conditional posture.
  v2: frozen LL = ll_random_hl_finetune (LL trained with HL ~ Uniform[-1,+1]^2).
      Result: HL action distribution widened (std 0.45 vs v1's 0.34) but
      net_gap_SB_minus_Bull still ~0; HL learned "be more long" globally
      rather than regime-conditional posture. On real_test, HL+LL underperformed
      LL alone (eq 1.217 vs 1.531).
  v3: frozen LL = ll_regime_bucket_hl_finetune ft 100k. LL was trained
      ONLY on regime-appropriate HL actions (disjoint corner buckets), so
      bucket centers produce clean regime-conditional weights:
        Bull bucket center  -> Bull-typed weights, eq 1.524
        Crisis bucket center -> Crisis-typed weights, eq 0.949
      Hypothesis: with an LL that has clean per-bucket structure, the HL
      gradient signal for "go to Bull bucket in Bull regime, Crisis bucket
      in Crisis regime" is strong enough that the HL learns regime-conditional
      posture for the first time.

No Fix A patch (LL already trained on full HL action distribution).
No HL action constraint (Option 1 — free exploration in [-1, +1]^2).

Run from project root:
    python train/hl_synth_pretrain_v3.py
"""
...
# Frozen LL — bucket-trained 100k checkpoint
LL_MODEL_PATH = (PATHS.checkpoints / "ll_regime_bucket_hl_finetune"
                 / "ppo_ll_regime_bucket_ft_100000_steps.zip")
LL_VECNORM_PATH = (PATHS.checkpoints / "ll_regime_bucket_hl_finetune"
...
def load_frozen_ll():
    """Load bucket-trained LL with its VecNormalize stats. NO Fix A patch
    (the LL was trained with HL actions across the full diagonal of
    [-1,+1]^2 via regime buckets, so its action-slot stats are valid)."""
...
class PerRegimePostureCallback(BaseCallback):
    """Per-regime gross/net diagnostic. Same as v1/v2 — every eval_freq steps,
    runs deterministic episode on real test from day 0 and logs per-regime
    mean gross and net.

    Bucket-aware extension: also logs whether the HL's actions land in the
    'correct' bucket for each regime. If the HL learned regime-conditional
    posture, we should see Bull-regime steps producing HL actions in the
    [+0.5, +1.0] x [+0.5, +1.0] bucket, etc.
    """

    # Bucket boundaries — same as in env.portfolio_hrl_env_regime_bucket
    BUCKETS = {
    ...
    def _is_in_bucket(self, hl_action, regime_idx):
        gl, gh, nl, nh = self.BUCKETS[regime_idx]
        g, n = float(hl_action[0]), float(hl_action[1])
        return (gl <= g <= gh) and (nl <= n <= nh)
    ...
    def _on_step(self) -> bool:
        ...
        while not done:
            ...
            hl_actions_history.append(np.asarray(action))
            done = terminated or truncated

        gross_arr = np.array(gross_history)
        net_arr = np.array(net_history)
        hl_arr = np.array(hl_actions_history)
        T = len(gross_arr)
        regime_slice = self.regime_idx[:T]

        # Per-regime gross/net (same as before)
        ...
        # Bucket-correctness — fraction of steps where HL action falls into
        # the regime-correct bucket. This is the v3-specific diagnostic.
        bucket_correct_overall = 0
        for t in range(T):
            r = int(regime_slice[t])
            if self._is_in_bucket(hl_arr[t], r):
                bucket_correct_overall += 1
        self.logger.record("bucket/correct_fraction_overall",
                           float(bucket_correct_overall / T))

        for i, name in enumerate(self.regime_names):
            mask = regime_slice == i
            if mask.sum() > 0:
                hl_in_regime = hl_arr[mask]
                in_bucket = sum(self._is_in_bucket(h, i) for h in hl_in_regime)
                self.logger.record(f"bucket/correct_fraction_{name}",
                                   float(in_bucket / mask.sum()))
        ...
        if self.verbose > 0:
            print(f"\n[posture @ step {self.num_timesteps}] "
                  ...
                  f"bucket-correct overall={bucket_correct_overall/T:.2f}")
            for i, name in enumerate(self.regime_names):
                mask = regime_slice == i
                if mask.sum() > 0:
                    hl_in_regime = hl_arr[mask]
                    ...
                    print(f"  {name:<11s} n={mask.sum():>3d}  "
                          ...
                          f"in-bucket={in_bucket}/{mask.sum()}={in_bucket/mask.sum():.2f}")
...
def main():
    print("=" * 80)
    print("HL synth pretrain v3 — frozen LL = ll_regime_bucket_hl_finetune (100k)")
    print("=" * 80)
    ...
    ckpt_dir = PATHS.checkpoints / "hl_synth_pretrain_v3"
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    tb_dir = PATHS.tb_logs / "hl_synth_pretrain_v3"
    ...
    try:
        ...
        final_model_path = ckpt_dir / "hl_v3_final.zip"
        final_vecnorm_path = ckpt_dir / "hl_v3_final_vecnorm.pkl"
    ...
    print()
    print("=" * 80)
    print("HL synth pretrain v3 complete.")
    print("=" * 80)
